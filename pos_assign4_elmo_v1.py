...
#    norm_embed = BatchNormalization()(embed_layer)
    LSTM1 = Bidirectional(LSTM(150, return_sequences=True, dropout=dropout_rate))(embed_layer)
    LSTM2 = Bidirectional(LSTM(150, return_sequences=True, dropout=dropout_rate))(LSTM1)
...
#    x = concatenate([gru_l1, gru_l2]) 
    ...
    hidden_layer1 = Dense(units=DENSE)(x)
...
lr_reducer = keras.callbacks.ReduceLROnPlateau(factor = 0.2, patience = 1, min_lr = 1e-6, verbose = 1)
check_pointer = keras.callbacks.ModelCheckpoint(model_file_name, verbose = 1, save_best_only = True)
early_stopper = keras.callbacks.EarlyStopping(patience = 4) # Change 4 to 8 in the final run    
